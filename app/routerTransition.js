...
class RouteTransitioner {
    ...
    filterAndSortURLs(count, latencies) {
        let urls = SettingsStore.getState()
            ...
            */
                if (!__TESTNET__ && a.url.indexOf("testnet") !== -1)
                    return false;
                /* Also remove the automatic fallback dummy url */
                if (a.url.indexOf("fake.automatic-selection") !== -1)
                    return false;
        ...
        Promise.all([connectionCheckPromise])
            .then(res => {
                ...
                if (appInit) {
                    ...
                    this.oldChain = "old";
                    Apis.reset(this.connectionManager.url, true).then(
                        instance => {
