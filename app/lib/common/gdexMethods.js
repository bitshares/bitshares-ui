...
export function fetchUserInfo(reqBody,timeout=-1){
    return requestSimple(reqBody, gdexAPIs.BASE + gdexAPIs.GET_USER_INFO, timeout);
}

export function validateAddress(reqBody, timeout=-1){
    if(!reqBody.address) return new Promise((res) => res());
    return requestSimple(reqBody, gdexAPIs.BASE + gdexAPIs.CHECK_WITHDRAY_ADDRESS, timeout);
}

export function requestDepositAddress(reqBody, timeout=-1){
    return requestSimple(reqBody, gdexAPIs.BASE + gdexAPIs.GET_DEPOSIT_ADDRESS, timeout);
}
...
export function getDepositRecordList(reqBody, timeout=-1){
    return requestSimple(reqBody, gdexAPIs.BASE + gdexAPIs.DEPOSIT_RECORD_LIST, timeout);
}

export function getWithdrawRecordList(reqBody, timeout=-1){
    return requestSimple(reqBody, gdexAPIs.BASE + gdexAPIs.WITHDRAW_RECORD_LIST, timeout);
}
