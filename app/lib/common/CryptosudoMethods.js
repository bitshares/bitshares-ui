...
import {cryptosudoAPIs} from "api/apiConfig";
import {availableGateways} from "common/gateways";
const cryptosudoStorage = new ls("");
...
export function fetchCoins(url = cryptosudoAPIs.BASE + cryptosudoAPIs.COINS_LIST) {
...
export function getDepositAddress({coin, account, stateCallback}) {
    ...
    fetch(cryptosudoAPIs.BASE + "/simple-api/get-last-address", {
        method: "POST",
        headers: new Headers({
...
function hasWithdrawalAddress(wallet) {
    return cryptosudoStorage.has(`history_address_${wallet}`);
}

function setWithdrawalAddresses({wallet, addresses}) {
    cryptosudoStorage.set(`history_address_${wallet}`, addresses);
}

function getWithdrawalAddresses(wallet) {
    return cryptosudoStorage.get(`history_address_${wallet}`, []);
}

function setLastWithdrawalAddress({wallet, address}) {
    cryptosudoStorage.set(`history_address_last_${wallet}`, address);
}

function getLastWithdrawalAddress(wallet) {
    return cryptosudoStorage.get(`history_address_last_${wallet}`, "");
}
