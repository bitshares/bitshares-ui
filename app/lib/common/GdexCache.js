...
class GdexCache {
    ...
    getIndexForDepositKey(account_name, input_coin_type, output_coin_type) {
        let args = [this.current_gdex_address_cache_version_string, account_name, input_coin_type, output_coin_type];
    ...
    getCachedInputAddress(account_name, input_coin_type, output_coin_type) {
        const index = this.getIndexForDepositKey(account_name, input_coin_type, output_coin_type);
    ...
    cacheInputAddress(account_name, input_coin_type, output_coin_type, address, memo) {
        const index = this.getIndexForDepositKey(account_name, input_coin_type, output_coin_type);
        let deposit_keys = session.get("deposit_keys", {});
        deposit_keys[index] = {"address": address, "memo": memo, "ctime": Math.floor(new Date().getTime() / 1000)};
