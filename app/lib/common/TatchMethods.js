...
export function fetchCoins(url = tatchAPIs.BASE + tatchAPIs.COINS_LIST) {
...
export function getDepositAddress({coin, account, stateCallback}) {
    ...
    fetch(tatchAPIs.BASE + "/simple-api/get-last-address", {
        method: "POST",
        headers: new Headers({
...
function hasWithdrawalAddress(wallet) {
    return tatchStorage.has(`history_address_${wallet}`);
}

function setWithdrawalAddresses({wallet, addresses}) {
    tatchStorage.set(`history_address_${wallet}`, addresses);
}

function getWithdrawalAddresses(wallet) {
    return tatchStorage.get(`history_address_${wallet}`, []);
}

function setLastWithdrawalAddress({wallet, address}) {
    tatchStorage.set(`history_address_last_${wallet}`, address);
}

function getLastWithdrawalAddress(wallet) {
    return tatchStorage.get(`history_address_last_${wallet}`, "");
}
