...
export default class MarketStatsCheck extends React.Component {
    ...
    shouldComponentUpdate(np) {
        ...
        const coreAsset = np.coreAsset.get("symbol");

        if (!fromAssets && fromAsset) fromAssets = [fromAsset];
        const fromMarkets = fromAssets.map(asset => {
            if (!asset) return null;
            return asset.get("symbol") + "_" + coreAsset;
        }).filter(a => !!a);
        ...
        const toAsset = np.toAsset.get("symbol");
        const toMarket = toAsset === coreAsset ? false : `${toAsset}_${coreAsset}`;
        return (
            fromCheck ||            
            toMarket && this._checkStats(np.marketStats.get(toMarket), this.props.marketStats.get(toMarket))
        );
    }
}
