...
export class DecimalChecker extends React.Component {
    ...
    onPaste(e) {
        ...
        if (decimalCount > 1)
            e.preventDefault();
        if (parseFloat(pasteValue) != pasteValue)
            e.preventDefault();

    }

    onKeyPress(e) {
        if (!e.nativeEvent.ctrlKey) { // allow copy-paste
            ...
            var decimalCount = decimal ? decimal.length : 0;
            if (e.key === "." && decimalCount > 1)
                e.preventDefault();
            if (parseFloat(nextValue) != nextValue)
                e.preventDefault();
...
class ExchangeInput extends DecimalChecker {
    ...
    render() {
        return <input ref="input" type="text" {...this.props} onPaste={this.onPaste.bind(this)} onKeyPress={this.onKeyPress.bind(this)} />;
    }
}
